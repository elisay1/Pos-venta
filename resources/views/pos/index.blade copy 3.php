    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="text-center info">
                    <h6>SPC Salón de Belleza</h6>
                    <p class="mb-0">Phone Number: +1 5656665656</p>
                    <p class="mb-0">Email: <a href="mailto:info@spcsalondebelleza.com">info@spcsalondebelleza.com</a></p>
                </div>
                <div class="tax-invoice">
                    <h6 class="text-center">Tax Invoice</h6>
                    <div class="row">
                        <div class="col-sm-12 col-md-6">
                            <div class="invoice-user-name"><span>Name: </span><span id="clienteNombre">John Doe</span></div>
                            <div class="invoice-user-name"><span>Invoice No: </span><span id="numeroVenta">CS132453</span></div>
                            <div class="invoice-user-name"><span>Seller: </span><span id="vendedorNombre">{{ Auth::user()->name . ' ' . Auth::user()->apellido }}</span></div>
                        </div>
                        <div class="col-sm-12 col-md-6">
                            <div class="invoice-user-name"><span>Customer Id: </span><span id="clienteDocumento">#LL93784</span></div>
                            <div class="invoice-user-name"><span>Date: </span><span id="fechaVenta">01.07.2022</span></div>
                            <div class="invoice-user-name"><span>Payment Method: </span><span id="metodoPago">Cash</span></div>
                        </div>
                    </div>
                </div>
                <table class="table-borderless w-100 table-fit">
...
<script src="https://cdn.jsdelivr.net/npm/quagga/dist/quagga.min.js"></script>
    ...
    <script>
        ...
        $('#btn_realizarCompra').on('click', function() {
            var clienteNombre = $('#client-nombre-completo').val();
            var clienteDocumento = $('#client-documento').val();
            var clienteCorreo = $('#client-correo').val();
            var comentario = $('#vent_coment').val();
            var subtotal = parseFloat($('#subTotal').text().replace('S/.', '').trim());
            var igv = parseFloat($('#tablaProductosCarrito').find('td').eq(8).text().replace('S/.', '').trim());  // Obtener el IGV correctamente
            var total = parseFloat($('#total').text().replace('S/.', '').trim());
            var descuento = parseFloat($('#descuento').text().replace('S/.', '').trim());
            ...
            var productos = carrito.map(function(product) {
                return {
                    ...
                    precio_unitario: product.costo_venta,
                    cantidad: product.cantidad,
                    subtotal: (product.costo_venta * product.cantidad).toFixed(2)
                };
            });
            ...
            $.ajax({
                ...
                data: JSON.stringify({
                    ...
                    subtotal: subtotal,
                    igv: igv || 0,  // Asegúrate de enviar 0 si igv es null
                    total: total,
                    descuento: descuento,
                ...
                success: function(response) {
                    ...
                    $('#clienteDocumento').text(clienteDocumento);
                    $('#fechaVenta').text(new Date().toLocaleDateString());
                    $('#metodoPago').text(metodoPago);
                    $('#numeroVenta').text(response.venta_id);
                    ...
                    $('#tablaProductosFactura').html(productosHtml);

                    $('#barcode').attr('src', 'data:image/png;base64,' + generarCodigoBarras(response.venta_id));
        ...
        function obtenerMetodoPago() {
            // Aquí puedes obtener el método de pago seleccionado por el usuario
            // Ejemplo: return $('#metodoPagoSelect').val();
            return 'Efectivo'; // Cambia esto por la lógica real
        }

        function generarCodigoBarras(ventaId) {
            return '{{ DNS1D::getBarcodePNG("' + ventaId + '", "C128", 2, 50) }}';
        }
